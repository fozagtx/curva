...
// The gate card for the prediction game: connect wallet -> live offer ->
// active call countdown -> result.
...
export default function PulseCall({ meta, probs, phase, gameApi }: Props) {
      ...
      <CardBody className="gap-4 p-4 sm:p-6">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-medium font-semibold">Pulse Calls</p>
        ...
        {!pubkey ? (
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            <p className="text-small text-default-500">
              Connect your Solana wallet to lock in calls and climb the board.
            </p>
            <Button
              color="primary"
...
function ActiveCallView({
  activeCall, latest, meta, progress,
}: {
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-small text-default-500">
          ...
          <span className={cn("font-semibold", activeCall.side === "higher" ? "text-primary" : "text-secondary")}>
        ...
        <Chip
          ...
          size="sm"
          startContent={
            <Icon icon={winning ? "solar:alt-arrow-up-bold" : "solar:alt-arrow-down-bold"} width={14} />
          }
          variant="flat"
        >
      ...
      <Progress aria-label="Time to resolution" color={winning ? "primary" : "danger"} size="sm" value={progress * 100} />
      <p className="text-tiny text-default-400">Resolves on the market&apos;s clock — watch the wave.</p>
    </div>
  );
}

function ResultView({ call, compact }: { call: NonNullable<UseGameResult["lastResolved"]>; compact?: boolean }) {
  ...
  return (
    <div
      className={cn(
        ...
        won ? "border-primary-100 bg-primary-50" : push ? "border-default-100 bg-default-50" : "border-danger-100 bg-danger-50",
        compact && "p-2",
      )}
    >
      <Icon
        className={won ? "text-primary" : push ? "text-default-500" : "text-danger"}
        icon={won ? "solar:cup-star-bold" : push ? "solar:minus-circle-bold" : "solar:close-circle-bold"}
        width={20}
      />
      <p className="text-small">
        {won ? (
          <>Called it. <span className="font-mono font-semibold">+{call.points}</span> pts on {call.teamName}.</>
        ) : push ? (
